...
try {
    ...
    // --- [NOVO] ROTAS PARA ORDEM DE SERVIÇO (OS) ---

    app.post('/api/os/generate', async (req, res) => {
        const { pointIds, responsavelOSId, observacoes, companyId, criadoPorUserId } = req.body;

        if (!companyId || !criadoPorUserId) {
            return res.status(400).json({ message: 'companyId e criadoPorUserId são obrigatórios.' });
        }
        if (!Array.isArray(pointIds) || pointIds.length === 0) {
            return res.status(400).json({ message: 'Pelo menos um ponto de instalação deve ser selecionado.' });
        }
        if (!responsavelOSId) {
        ...
        try {
            const newOS = await db.runTransaction(async (transaction) => {
                ...
                if (counterDoc.exists) {
                    ...
                    transaction.update(counterRef, { lastSeq: newSeq });
                } else {
                    transaction.set(counterRef, { lastSeq: newSeq });
                }

                const numeroOS = `OS-${year}-${String(newSeq).padStart(3, '0')}`;

                // Validar todos os pontos antes de prosseguir
                const pointRefs = pointIds.map(id => db.collection('instalacaoPontos').doc(id));
                const pointDocs = await transaction.getAll(...pointRefs);

                for (const pointDoc of pointDocs) {
                    if (!pointDoc.exists) {
                        throw new Error(`O ponto com ID ${pointDoc.id} não foi encontrado.`);
                    }
                    const pointData = pointDoc.data();
                    if (pointData.status !== 'Planejado') {
                        throw new Error(`Um dos pontos selecionados já está em outra OS ou foi cancelado.`);
                    }
                    if (pointData.companyId !== companyId) {
                        throw new Error(`O ponto com ID ${pointDoc.id} não pertence à sua empresa.`);
                    }
                }
                ...
                const osData = {
                    ...
                    prazoExecucao: null, // TODO: Implementar cálculo de prazo na Parte 3
                    status: "Planejada",
                ...
                // Atualizar os pontos selecionados
                pointDocs.forEach(pointDoc => {
                    transaction.update(pointDoc.ref, {
                        osId: newOSRef.id,
                        status: "EmOS",
                        responsavelId: responsavelOSId // Regra: sobrescreve o responsável
                    });
                });

                return { id: newOSRef.id, ...osData };
            });

            res.status(201).json({ message: 'OS gerada com sucesso!', os: newOS });

        } catch (error) {
            console.error("Erro transacional ao gerar OS:", error);
            res.status(500).json({ message: `Falha ao gerar OS: ${error.message}` });
        }
    });

    app.get('/api/os/list', async (req, res) => {
        const { companyId, year, status, responsavelOSId } = req.query;
        if (!companyId) {
            return res.status(400).json({ message: 'O ID da empresa é obrigatório.' });
        }
        try {
            ...
            if (year) {
                query = query.where('ano', '==', parseInt(year, 10));
            }
            if (status) {
                query = query.where('status', '==', status);
            }
            if (responsavelOSId) {
                query = query.where('responsavelOSId', '==', responsavelOSId);
            }
