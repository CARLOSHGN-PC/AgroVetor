...
try {
    ...
    // Criar um novo ponto de instalação
    app.post('/api/instalacao/pontos', async (req, res) => {
        try {
            const { id, ...data } = req.body;
            if (!data.companyId || !data.fazendaId || !data.talhaoId || !data.coordenadas) {
                return res.status(400).json({ message: 'Dados incompletos para criar ponto de instalação.' });
            }
            const docRef = id ? db.collection('instalacaoPontos').doc(id) : db.collection('instalacaoPontos').doc();
            await docRef.set({
                ...data,
                criadoEm: admin.firestore.FieldValue.serverTimestamp(),
                status: 'Planejado'
            }, { merge: true });
            res.status(201).json({ id: docRef.id, message: 'Ponto de instalação salvo com sucesso.' });
        } catch (error) {
    ...
    // Buscar todos os pontos com status "Planejado"
    app.get('/api/instalacao/pontos/planejados', async (req, res) => {
        ...
        try {
            ...
            console.error("Erro ao buscar pontos planejados:", error);
            res.status(500).json({ message: 'Erro no servidor ao buscar pontos.' });
        }
    });

    // Gerar uma nova Ordem de Serviço (OS)

    // Gerar uma nova Ordem de Serviço (OS)
    app.post('/api/instalacao/ordens-de-servico', async (req, res) => {
        const { companyId, pontosIds, responsavelOSId, observacoes } = req.body;
        if (!companyId || !pontosIds || !responsavelOSId || pontosIds.length === 0) {
            return res.status(400).json({ message: 'Dados incompletos para gerar OS.' });
        }
        ...
        try {
            const osId = await db.runTransaction(async (transaction) => {
                const counterDoc = await transaction.get(counterRef);
                let nextSeq = 1;
                if (counterDoc.exists) {
                    nextSeq = (counterDoc.data().lastSeq || 0) + 1;
                }
                transaction.set(counterRef, { lastSeq: nextSeq }, { merge: true });

                const numeroOS = `OS-${year}-${String(nextSeq).padStart(3, '0')}`;
                const osRef = db.collection('instalacaoOrdensDeServico').doc();

                const prazo = calculatePrazoExecucao();
                ...
                transaction.set(osRef, {
                    ...
                    prazoExecucao: admin.firestore.Timestamp.fromDate(prazo),
                    status: 'Planejada',
                    criadoEm: admin.firestore.FieldValue.serverTimestamp(),
                    criadoPorUserId: req.user?.uid || null // Assumindo que um middleware de auth populará req.user
                });

                // Atualiza o status dos pontos para "EmOS"
                pontosIds.forEach(pontoId => {
            ...
            res.status(201).json({ id: osId, message: 'Ordem de Serviço gerada com sucesso.' });

        } catch (error) {
            console.error("Erro ao gerar Ordem de Serviço:", error);
            res.status(500).json({ message: 'Erro no servidor ao gerar OS.' });
        }
    });


    // Marcar um ponto como instalado e criar armadilha
    app.put('/api/instalacao/pontos/:id/instalar', async (req, res) => {
        const { id } = req.params;
        const { companyId, concluidoPorUserId, fotoURL, observacoes, fazendaNome, talhaoNome, fazendaCode } = req.body;

        if (!companyId || !concluidoPorUserId) {
            return res.status(400).json({ message: 'Dados incompletos para instalar o ponto.' });
        }
        ...
        try {
            ...
            await db.runTransaction(async (transaction) => {
                ...
                transaction.update(pontoRef, {
                    ...
                    observacoes: observacoes || pontoData.observacoes || ''
                });

                // 2. Cria a armadilha correspondente
                const armadilhaRef = db.collection('armadilhas').doc();
            ...
            res.status(500).json({ message: 'Erro no servidor ao instalar ponto.' });
        }
    });
