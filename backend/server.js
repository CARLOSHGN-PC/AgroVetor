...
try {
    ...
    app.post('/api/generate-os-number', async (req, res) => {
        ...
        const counterRef = db.collection('osCounters').doc(String(currentYear));
        ...
        try {
            const osNumber = await db.runTransaction(async (transaction) => {
                ...
                const seqPadded = String(newSeq).padStart(3, '0');
                return `OS-${currentYear}-${seqPadded}`;
            });

            res.status(200).json({ osNumber: osNumber });
        } catch (error) {
    ...
    const getInstalacaoData = async (filters) => {
        ...
        let query = db.collection('instalacaoPontos').where('companyId', '==', filters.companyId);

        // Fitler for installed points, as we are reporting on installation history
        query = query.where('status', '==', 'Instalado');
        ...
        if (filters.responsavelId) {
            query = query.where('responsavelId', '==', filters.responsavelId);
        }
        ...
        const farmIds = [...new Set(data.map(d => d.fazendaId))];
        const userIds = [...new Set(data.map(d => d.concluidoPorUserId).filter(Boolean))];

        let farmsMap = {};
        if (farmIds.length > 0) {
            // Firestore 'in' query is limited to 30 items. Chunking is necessary for larger queries.
            const farmChunks = [];
            for (let i = 0; i < farmIds.length; i += 30) {
                farmChunks.push(farmIds.slice(i, i + 30));
            }
            for (const chunk of farmChunks) {
                const farmsSnapshot = await db.collection('fazendas').where(admin.firestore.FieldPath.documentId(), 'in', chunk).get();
                farmsSnapshot.forEach(doc => {
                    farmsMap[doc.id] = doc.data();
                });
            }
        }
        ...
        let enrichedData = data.map(item => {
            const farmData = farmsMap[item.fazendaId];
            const userData = usersMap[item.concluidoPorUserId];
            return {
                ...item,
                fazendaNome: farmData?.name || 'N/A',
                fazendaCodigo: farmData?.code || 'N/A',
                concluidoPorNome: userData?.username || 'N/A',
            };
        });

        if (filters.fazendaId) {
             enrichedData = enrichedData.filter(d => d.fazendaId === filters.fazendaId);
        }


        return enrichedData.sort((a, b) => {
            const dateA = a.dataInstalacao ? a.dataInstalacao.toMillis() : 0;
            const dateB = b.dataInstalacao ? b.dataInstalacao.toMillis() : 0;
            if (dateA !== dateB) return dateA - dateB;
            return (a.fazendaCodigo || '').localeCompare(b.fazendaCodigo || '');
        });
    };
    ...
    app.get('/reports/instalacao/historico/pdf', async (req, res) => {
        ...
        try {
            ...
            const headers = ['Data Instalação', 'Fazenda', 'Talhão', 'Responsável', 'OS', 'Obs'];
            const columnWidths = [100, 200, 100, 150, 100, 132];
            ...
            for (const item of data) {
                ...
                const row = [
                    item.dataInstalacao ? safeToDate(item.dataInstalacao).toLocaleDateString('pt-BR') : 'N/A',
                    `${item.fazendaCodigo} - ${item.fazendaNome}`,
                    item.talhaoId, // Assuming talhaoId is the name/code for now
                    item.concluidoPorNome,
                    item.osId || 'N/A',
                    item.descricao || ''
                ];
    ...
    app.get('/reports/instalacao/historico/csv', async (req, res) => {
        try {
            ...
            const filePath = path.join(os.tmpdir(), `instalacao_historico_${Date.now()}.csv`);
            const csvWriter = createObjectCsvWriter({
                path: filePath,
                header: [
                    { id: 'dataInstalacao', title: 'Data Instalação' },
                    { id: 'fazenda', title: 'Fazenda' },
                    { id: 'talhaoId', title: 'Talhão' },
                    { id: 'responsavel', title: 'Responsável' },
                    { id: 'osId', title: 'OS' },
                    { id: 'descricao', title: 'Observações' }
                ]
            });

            const records = data.map(item => ({
                dataInstalacao: item.dataInstalacao ? safeToDate(item.dataInstalacao).toLocaleDateString('pt-BR') : 'N/A',
                fazenda: `${item.fazendaCodigo} - ${item.fazendaNome}`,
                talhaoId: item.talhaoId,
                responsavel: item.concluidoPorNome,
                osId: item.osId || 'N/A',
