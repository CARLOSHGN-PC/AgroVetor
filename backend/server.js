...
try {
    ...
    app.post('/reports/clima/pdf', async (req, res) => {
        const doc = new PDFDocument({ margin: 30, size: 'A4', layout: 'landscape', bufferPages: true });
        res.setHeader('Content-Type', 'application/pdf');
        ...
        try {
            ...
            // --- Adicionar Gráficos ---
            if (chartImages) {
                currentY = await checkPageBreak(doc, currentY, title, 280); // Check for enough space for charts
                doc.fontSize(12).font('Helvetica-Bold').text('Gráficos de Indicadores', doc.page.margins.left, currentY);
                currentY += 15;

                const chartWidth = (doc.page.width - doc.page.margins.left - doc.page.margins.right - 20) / 2;
                const chartHeight = 220;

                const chartKeys = ['tempChart1', 'tempChart2', 'tempChart3', 'tempChart4'];
                if (chartImages[chartKeys[0]]) {
                    doc.image(chartImages[chartKeys[0]], doc.page.margins.left, currentY, { width: chartWidth });
                }
                if (chartImages[chartKeys[1]]) {
                    doc.image(chartImages[chartKeys[1]], doc.page.margins.left + chartWidth + 20, currentY, { width: chartWidth });
                }
                currentY += chartHeight + 10;
                 if (chartImages[chartKeys[2]]) {
                    doc.image(chartImages[chartKeys[2]], doc.page.margins.left, currentY, { width: chartWidth });
                }
                if (chartImages[chartKeys[3]]) {
                    doc.image(chartImages[chartKeys[3]], doc.page.margins.left + chartWidth + 20, currentY, { width: chartWidth });
                }
                currentY += chartHeight + 20;
            }

            currentY = await checkPageBreak(doc, currentY, title, 100);
            doc.fontSize(12).font('Helvetica-Bold').text('Dados Tabulados', doc.page.margins.left, currentY);
            currentY += 15;


            const headers = ['Data', 'Fazenda', 'Talhão', 'Temp. Máx (°C)', 'Temp. Mín (°C)', 'Umidade (%)', 'Pluviosidade (mm)', 'Vento (km/h)', 'Obs'];
            const columnWidths = [65, 150, 80, 70, 70, 70, 80, 70, 127];
            ...
            let totalUmidade = 0;
            let totalVento = 0;
            let recordCount = data.length > 0 ? data.length : 1;
            ...
            for (const record of data) {
                ...
                const row = [
                    ...
                    `${record.farmCode} - ${record.farmName}`,
                    record.talhao,
                    formatNumber(record.temperaturaMaxima),
                    formatNumber(record.temperaturaMinima),
                    formatNumber(record.umidadeRelativa),
                    formatNumber(record.pluviosidade),
                    formatNumber(record.velocidadeVento),
                    record.observacoes || ''
                ];
                currentY = drawRow(doc, row, currentY, false, false, columnWidths);

                totalPluviosidade += record.pluviosidade || 0;
                totalTempMax += record.temperaturaMaxima || 0;
                totalTempMin += record.temperaturaMinima || 0;
                totalUmidade += record.umidadeRelativa || 0;
                totalVento += record.velocidadeVento || 0;
            }

            currentY += 15;
            currentY = await checkPageBreak(doc, currentY, title, 80);

            doc.fontSize(10).font('Helvetica-Bold').text('Resumo dos Indicadores', doc.page.margins.left, currentY);
            currentY += 15;

            const summaryData = [
                ['Total de Pluviosidade (mm)', formatNumber(totalPluviosidade)],
                ['Média de Temp. Máxima (°C)', formatNumber(totalTempMax / recordCount)],
                ['Média de Temp. Mínima (°C)', formatNumber(totalTempMin / recordCount)],
                ['Média de Umidade Relativa (%)', formatNumber(totalUmidade / recordCount)],
                ['Média de Velocidade do Vento (km/h)', formatNumber(totalVento / recordCount)],
            ];

            summaryData.forEach(item => {
                doc.font('Helvetica-Bold').text(item[0] + ':', doc.page.margins.left, currentY, { continued: true, width: 250 });
                doc.font('Helvetica').text(item[1], doc.page.margins.left + 260, currentY);
                currentY += 15;
            });
    ...
    app.get('/reports/clima/csv', async (req, res) => {
        try {
            ...
            console.error("Erro ao gerar CSV Climatológico:", error);
            res.status(500).send('Erro ao gerar relatório.');
        }
    });
