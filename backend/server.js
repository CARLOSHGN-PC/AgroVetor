...
try {
    ...
    app.get('/reports/perobox/pdf', async (req, res) => {
        const doc = new PDFDocument({ margin: 30, size: 'A4', layout: 'landscape', bufferPages: true });
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename=relatorio_perobox.pdf`);
        doc.pipe(res);

        try {
            const filters = req.query;
            const data = await getFilteredData('perobox', filters);
            const title = 'Relatório de Instalação e Coleta Perobox';
            ...
            const headers = ['Tipo', 'Data', 'Fazenda', 'Talhão', 'Variedade', 'Ponto', 'Mariposas', 'Observação do Ponto', 'Observação Geral'];
            const columnWidths = [60, 65, 120, 80, 80, 40, 60, 150, 127];

            currentY = drawRow(doc, headers, currentY, true, false, columnWidths);

            for (const item of data) {
                const dataPrincipal = item.dataColeta ? new Date(item.dataColeta).toLocaleDateString('pt-BR') : new Date(item.data + 'T03:00:00Z').toLocaleDateString('pt-BR');

                if (item.pontos && item.pontos.length > 0) {
                    for (let i = 0; i < item.pontos.length; i++) {
                        const ponto = item.pontos[i];
                        const rowData = [
                            i === 0 ? item.tipo : '',
                            i === 0 ? dataPrincipal : '',
                            i === 0 ? `${item.fazendaId} - ${item.fazendaNome}` : '',
                            i === 0 ? item.talhaoNome : '',
                            i === 0 ? item.variedade : '',
                            i + 1,
                            ponto.mariposas,
                            ponto.observacao || '',
                            i === 0 ? item.observacaoGeral || '' : ''
                        ];
                        currentY = await checkPageBreak(doc, currentY, title);
                        currentY = drawRow(doc, rowData, currentY, false, false, columnWidths);
                    }
                     ...
                     const rowData = [
                        item.tipo,
                        dataPrincipal,
                        `${item.fazendaId} - ${item.fazendaNome}`,
                        item.talhaoNome,
                        item.variedade,
                        'N/A',
                        'N/A',
    ...
    app.get('/reports/perobox/csv', async (req, res) => {
        try {
            ...
            const filePath = path.join(os.tmpdir(), `perobox_${Date.now()}.csv`);
            const csvWriter = createObjectCsvWriter({
                path: filePath,
                header: [
                    { id: 'tipo', title: 'Tipo' },
                    ...
                    { id: 'mariposas', title: 'Mariposas' },
                    { id: 'obsPonto', title: 'Observação do Ponto' },
                    { id: 'obsGeral', title: 'Observação Geral' }
                ]
            });

            const records = [];
            data.forEach(item => {
                const dataPrincipal = item.dataColeta || item.data;
                 if (item.pontos && item.pontos.length > 0) {
                    item.pontos.forEach((ponto, index) => {
                        records.push({
                            tipo: item.tipo,
                            data: dataPrincipal,
                            fazenda: `${item.fazendaId} - ${item.fazendaNome}`,
                            talhao: item.talhaoNome,
                            variedade: item.variedade,
                            ponto: index + 1,
                            mariposas: ponto.mariposas,
                            obsPonto: ponto.observacao,
                            obsGeral: item.observacaoGeral
                        });
                    });
                 }
            });
