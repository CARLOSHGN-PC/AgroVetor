...
public class NativeAerialMapOverlay implements OnMapClickListener {
    ...
    private static WeakReference<NativeAerialMapOverlay> activeInstance = new WeakReference<>(null);
    ...
    public NativeAerialMapOverlay(@NonNull android.app.Activity activity, @NonNull FrameLayout container) {
        this.hostActivity = activity;
        this.hostContainer = container;
    }

    public void attach() {
        Log.i(TAG, "NativeAerialMapOverlay.attach");
        if (mapView == null) {
        ...
        hostContainer.setVisibility(android.view.View.VISIBLE);
        activeInstance = new WeakReference<>(this);
        mapView.onStart();
    }

    public void detach() {
        Log.i(TAG, "NativeAerialMapOverlay.detach");
        NativeAerialMapOverlay current = activeInstance.get();
        if (current == this) {
        ...
        hostContainer.setVisibility(android.view.View.GONE);
    }

    public void destroy() {
        Log.i(TAG, "NativeAerialMapOverlay.destroy");
        if (gesturesPlugin != null) {
            gesturesPlugin.removeOnMapClickListener(this);
            gesturesPlugin = null;
        }
    ...
    private void createMapViewWithOfflineRuntime() {
        FrameLayout mapContainer = hostContainer;
        String accessToken = hostActivity.getString(R.string.mapbox_access_token);
        AerialMapboxRuntime.configureMapbox(hostActivity.getApplicationContext(), accessToken);
        MapInitOptions mapInitOptions = new MapInitOptions(hostActivity);
        mapView = new MapView(hostActivity, mapInitOptions);
    ...
    public static void updateCameraIfVisible(double[] center, double zoom) {
        ...
        current.hostActivity.runOnUiThread(() -> current.mapView.getMapboxMap().setCamera(
    ...
    public static void reloadTalhoesIfVisible(String geojson) {
        ...
        current.hostActivity.runOnUiThread(() -> {
    ...
    public static void highlightTalhaoIfVisible(String talhaoId) {
        ...
        current.hostActivity.runOnUiThread(() -> current.mapView.getMapboxMap().getStyle(style -> current.applyHighlight(style, talhaoId)));
    }
    ...
    private String buildDetailedFailureReason(String reasonType, String styleUri) {
        List<OfflineRegionMetadata> regions = new AerialOfflineRegionStore(hostActivity.getApplicationContext()).readAll();
    ...
    private List<OfflineRegionMetadata> readOfflineMetadata(boolean networkAvailable) {
        AerialOfflineRegionStore regionStore = new AerialOfflineRegionStore(hostActivity.getApplicationContext());
    ...
    private void prepareStyleFallbackAndLoad(boolean networkAvailable, List<OfflineRegionMetadata> metadataRegions) {
        Thread worker = new Thread(() -> {
            RuntimeOfflineSnapshot snapshot = RuntimeOfflineSnapshot.resolve(hostActivity.getApplicationContext());
    ...
    private boolean isNetworkAvailable() {
        return NetworkUtils.isNetworkAvailable(hostActivity.getApplicationContext());
    }

    private void registerPluginErrorForwarder() {
        AerialMapPlugin.setNativeErrorHandler((message, details) -> hostActivity.runOnUiThread(() -> {
