...
public class NativeAerialMapFragment extends Fragment implements OnMapClickListener {
    ...
    private static WeakReference<NativeAerialMapFragment> activeInstance = new WeakReference<>(null);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        Log.i(TAG, "NativeAerialMapFragment.onCreateView");
        return inflater.inflate(R.layout.activity_aerial_map, container, false);
    }

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        Log.i(TAG, "NativeAerialMapFragment.onViewCreated");

        createMapViewWithOfflineRuntime(view);
    ...
    private void createMapViewWithOfflineRuntime(@NonNull View rootView) {
        FrameLayout mapContainer = rootView.findViewById(R.id.nativeAerialMapContainer);
        String accessToken = getString(R.string.mapbox_access_token);
        AerialMapboxRuntime.configureMapbox(requireContext().getApplicationContext(), accessToken);
        MapInitOptions mapInitOptions = new MapInitOptions(requireContext());
    ...
    public void onStart() {
        super.onStart();
        Log.i(TAG, "NativeAerialMapFragment.onStart");
    ...
    @Override
    public void onStop() {
        Log.i(TAG, "NativeAerialMapFragment.onStop");
        NativeAerialMapFragment current = activeInstance.get();
        if (current == this) {
    ...
    public static void updateCameraIfVisible(double[] center, double zoom) {
        ...
        current.requireActivity().runOnUiThread(() -> current.mapView.getMapboxMap().setCamera(
    ...
    public static void reloadTalhoesIfVisible(String geojson) {
        ...
        current.requireActivity().runOnUiThread(() -> {
    ...
    public static void highlightTalhaoIfVisible(String talhaoId) {
        ...
        current.requireActivity().runOnUiThread(() -> current.mapView.getMapboxMap().getStyle(style -> current.applyHighlight(style, talhaoId)));
    }
    ...
    private String buildDetailedFailureReason(String reasonType, String styleUri) {
        List<OfflineRegionMetadata> regions = new AerialOfflineRegionStore(requireContext().getApplicationContext()).readAll();
    ...
    private List<OfflineRegionMetadata> readOfflineMetadata(boolean networkAvailable) {
        AerialOfflineRegionStore regionStore = new AerialOfflineRegionStore(requireContext().getApplicationContext());
    ...
    private void prepareStyleFallbackAndLoad(boolean networkAvailable, List<OfflineRegionMetadata> metadataRegions) {
        Thread worker = new Thread(() -> {
            RuntimeOfflineSnapshot snapshot = RuntimeOfflineSnapshot.resolve(requireContext().getApplicationContext());
    ...
    private boolean isNetworkAvailable() {
        return NetworkUtils.isNetworkAvailable(requireContext().getApplicationContext());
    }

    private void registerPluginErrorForwarder() {
        AerialMapPlugin.setNativeErrorHandler((message, details) -> {
            if (!isAdded()) return;
            requireActivity().runOnUiThread(() -> {
            ...
            Log.i(TAG, "Erro nativo interceptado sem fechar fragmento.");
            });
        });
    }
    ...
    public void onDestroyView() {
        Log.i(TAG, "NativeAerialMapFragment.onDestroyView");
        if (gesturesPlugin != null) {
            gesturesPlugin.removeOnMapClickListener(this);
            gesturesPlugin = null;
        }
