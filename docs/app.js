...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const App = {
        ...
        ui: {
            ...
            showTab(id) {
                ...
                if (id === 'configuracoesEmpresa') {
                    const plantioMetaInput = document.getElementById('plantioMetaInput');
                    if (plantioMetaInput) {
                        plantioMetaInput.value = App.state.companyConfig?.plantioMeta || '';
                    }
                }
                if (id === 'cadastros') {
                    this.renderFarmSelect();
            ...
            setupEventListeners() {
                ...
                const btnSavePlantioMeta = document.getElementById('btnSavePlantioMeta');
                if (btnSavePlantioMeta) {
                    btnSavePlantioMeta.addEventListener('click', () => {
                        const plantioMetaInput = document.getElementById('plantioMetaInput');
                        const meta = parseFloat(plantioMetaInput.value);
                        if (isNaN(meta) || meta < 0) {
                            App.ui.showAlert("Por favor, insira um valor válido para a meta de plantio.", "error");
                            return;
                        }
                        App.actions.saveCompanyConfig({ plantioMeta: meta }, "Meta de plantio atualizada com sucesso!");
                    });
                }
        ...
        actions: {
            ...
            async saveCompanyConfig(configData, successMessage) {
                const companyId = App.state.currentUser.companyId;
                if (!companyId) {
                    App.ui.showAlert("Não foi possível identificar a empresa para salvar a configuração.", "error");
                    return;
                }
                App.ui.setLoading(true, "A guardar configuração...");
                try {
                    await App.data.setDocument('config', companyId, configData, { merge: true });
                    // Update local state immediately for responsiveness
                    App.state.companyConfig = { ...App.state.companyConfig, ...configData };
                    App.ui.showAlert(successMessage, "success");
                } catch (error) {
                    console.error("Erro ao guardar a configuração da empresa:", error);
                    App.ui.showAlert(`Erro ao guardar: ${error.message}`, 'error');
                } finally {
                    App.ui.setLoading(false);
                }
...
            },
            ...
            async renderPlantioDashboardCharts() {
                ...
                const metaPlantio = App.state.companyConfig?.plantioMeta || 0; // Use saved meta
