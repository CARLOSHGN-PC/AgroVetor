        ...
        mapModule: {
            ...
            async loadAndCacheShapes(url) {
                ...
                try {
                    ...
                    console.log('[SHP] features lidas:', featuresCount);
                    if (!featuresCount) {
                        App.ui.showAlert('O SHP foi carregado, mas não contém polígonos válidos para desenho.', 'warning', 7000);
                    }
                    if (debug) {
                        ...
                        console.log('[SHP] reprojeção pulada:', Boolean(debug.skippedReprojection));
                        if (debug.sourceStats) console.log('[SHP] stats origem:', debug.sourceStats);
                        if (debug.transformedStats) console.log('[SHP] stats reprojetado:', debug.transformedStats);
                ...
                try {
                    ...
                    if (buffer) {
                        ...
                        try {
                            ...
                            const featuresCountOffline = geojson?.features?.length || 0;
                            console.log('[SHP] features lidas (offline):', featuresCountOffline);
                            if (!featuresCountOffline) {
                                App.ui.showAlert('Mapa offline sem polígonos válidos para desenho.', 'warning', 7000);
                            }
                            if (debug) {
                                ...
                                console.log('[SHP] reprojeção pulada (offline):', Boolean(debug.skippedReprojection));
                                if (debug.sourceStats) console.log('[SHP] stats origem (offline):', debug.sourceStats);
                                if (debug.transformedStats) console.log('[SHP] stats reprojetado (offline):', debug.transformedStats);
            ...
            async runShapefileDebugTest() {
                ...
                try {
                    ...
                    console.log('[SHP DEBUG] reprojeção pulada =', Boolean(debug?.skippedReprojection));
                    if (debug?.sourceStats) console.log('[SHP DEBUG] stats origem =', debug.sourceStats);
                    if (debug?.transformedStats) console.log('[SHP DEBUG] stats reprojetado =', debug.transformedStats);
